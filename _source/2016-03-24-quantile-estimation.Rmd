...
4. Evolution of the bias with varying sample size
    ...
    * all methods underestimate the spread of the outer quantiles at low sample sizes ($$q_{0.125}$$ to $$q_{0.875}$$ noticeable biased for  $$n<5$$)
...
mtext(paste("running mean of repeated quantile estimation\n of random samples with n =", n, "of the standard normal distribution"), 3, outer=T, line=1, font=2, cex=1)
...
for(type in 1:9){
  ...
  plot(NULL, xlim=c(1,nmax), ylim=ylim, log="x", main=paste("type ", type), xlab="", ylab="", axes=F, frame.plot=T, panel.first=abline(v=c(1,2,5,10,20,50,100,200,500,1000), h=qnorm(Q[Qselect]), col="lightgray"))
  for(i in Qselect){
...
```{r difference-theoretical-to-estimated-quantiles, dev="svg", cache=F}
...
for(type in 1:9){
  ...
  for(i in seq_along(nn)){
    x <- seq(1/(nn[i]*2), 1-1/(nn[i]*2), length.out=nn[i])
    Qmatrix[type,i,] <- quantile(qnorm(x), Q, type = type)-qnorm(Q)
  }
}

ylim <- c(-1,1)*2.5
palette(viridis(length(nn), direction=-1, end=0.9))
# type=5
for(type in 1:9){
  ...
  if(type==6){
    legend("right", legend=as.character(nn), col = 1:length(nn), xpd = NA, lty=1, horiz = F, title="n =", inset = -0.45)
  }
}
...
for(type in 1:9){
  plot(NULL, xlim=c(0,1), ylim=ylim, panel.first = abline(h=0, col=1), axes=F, frame.plot=T, xlab="", ylab="", main=paste("type ", type))
...
```{r deviation-greater-than-tolerance-type-5, dev="svg", fig.height=5, cache=F}
par(mfrow=c(1,1), mar=c(0,1,2,.1), oma=c(5,4,3,6))
...
in_tolerance_low <- seq_along(nn)
in_tolerance_up <- seq_along(nn)
for(i in seq_along(nn)){
...
legend("right", legend=as.character(nn), col = 1:length(nn), xpd = NA, lty=1, horiz = F, title="n =", inset = -0.25)


mtext(paste0("deviation greater than tolerance"), 3, outer=T, line=1, font=2, cex=1)
mtext("deviation from theoretical quantiles", 2, outer=T, line=1.5)
mtext("probabilities", 1, outer=T, line=3)

```

```{r probabilities-tolerable-to-estimate-type-5, fig.height=5, dependson="deviation-greater-than-tolerance-type-5"}
plot(nn, in_tolerance_low, type="o", pch=20, main="type 5", ylab="probabilities tolerable to estimate", xlab="n", ylim=c(0,1), xlim=c(0,35), xaxt="n")
text(nn, in_tolerance_low, labels = round(in_tolerance_low, 2), adj = c(0,-1))
lines(nn, in_tolerance_up, type="o", pch=20)
text(nn, in_tolerance_up, labels = round(in_tolerance_up, 2), adj = c(0,1.8))
axis(1, at = nn)
```

```{r deviation-greater-than-tolerance-type-7, dev="svg", fig.height=5, cache=F}
par(mfrow=c(1,1), mar=c(0,1,2,.1), oma=c(5,4,3,6))
...
axis(2, las=1)
axis(1, at=in_tolerance_low[1:5], labels = round(in_tolerance_low, 2)[1:5], las=2)
axis(1, at=in_tolerance_up[1:5], labels = round(in_tolerance_up, 2)[1:5], las=2)
axis(1, at=c(0,1,0.5), las=2)
...
```{r deviation-tolerable-type-7, fig.height=5, cache=F}
plot(nn, in_tolerance_low, type="o", pch=20, main="type 7", ylab="probabilities tolerable to estimate", xlab="n", ylim=c(0,1), xlim=c(0,35), xaxt="n")
text(nn, in_tolerance_low, labels = round(in_tolerance_low, 2), adj = c(0,-1))
lines(nn, in_tolerance_up, type="o", pch=20)
text(nn, in_tolerance_up, labels = round(in_tolerance_up, 2), adj = c(0,1.8))
axis(1, at = nn)
