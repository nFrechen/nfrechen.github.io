...
opts_chunk$set(fig.height=7, echo=F, cache=F, dev="svg")
```
...
There are many different ways to estimate the quantiles of an empirical distribution. R knows 9 different types. All these methods where intruduced to optimize quantile estimation for sparse data sets, to get better estimations for the low $$n$$. It is clear that for large sample sizes the differences in these approaches become negligible. It is the low $$n$$ where they produce different estimations. There is a lot of mathematical argumentation and justification out there for each of these methods, but apparently none of them is convincing enough, that the whole statistical community would stick to that method. In this article we investigate the differences between these 9  quantile estimation methods and show how they perform especially for the low sample sizes of $$n<50$$ or even $$n<10$$ and how the deviations develope with increasing sample size and across the range of quantiles.
...
Because most statistical tools were developed in the pre-computer era, they tend to favour easy computability. Also they were proofed theoreticaly because that was the only method at that time.
...
[Since 2004](https://stat.ethz.ch/pipermail/r-announce/2004/000427.html) R has implemented 9 types of quantile estimation recommended by `r citet("Hyndman and Fan (1996)")`. The following is how they are described in the R documentation:
...
Type&nbsp;4: | `r citet(bib["parzen1979nonparametric"])` | $$m = 0$$ | $$p_k = \frac{k}{n}$$ | That is, linear interpolation of the empirical cdf.
...
The plotting positions of the continuous types described in the documentation and the paper of `r citet("Hyndman_1996")` are visualized in the next graph. The graph is sorted in the order of the parameter $$a$$ from $$a=0$$ of the `r citet(bib["weibull1939phenomenon"])` approach to $$a=1$$ of the `r citet(bib["gumbel1939probabilite"])` approach. For all we marked the positive (green) and negative (red)
difference to the `r citet(bib["hazen1914storage"])` approach which was not only proposed first, but will later play an important role in our analysis.

The method proposed by `r citet(bib["parzen1979nonparametric"])` stands out in that the plotting positions are not oriented around the probability of 0.5 as all the others do but instead are all shifted to the right. 
...
```{r potting-positions-figure, results="hide", cache=FALSE, eval=F}
system("pdf2svg ../../figure/source/2016-03-24-quantile-estimation/plotting-positions-1.pdf ../../figure/source/2016-03-24-quantile-estimation/plotting-positions-1.pdf")
```
...
The following graph shows the starting point of our investigation: The repeated application of quantile estimations on data samples generated by the R internal random generator `rnorm()` which produces normal distributed random data. For a single random sample the estimations are expected to show different results due to their random nature. But if the mean of repeated estimation of different random samples is calculated, these random differences should level out. 
The graph shows estimations done on samples of $$n=10$$ and the random variations indeed level out after $$\approx100$$ repetitions – but not at the expected theoretical quantiles that where programmed into the random generator. 6 of the 9 methods show severe deviations. Some positive, some negative. How do these differences develope whith varying sample size and how severe are the deviations over the whole range of quantiles? These are the questions we will investigate in the following. 
...
mtext(paste("of random samples with n =", n, "drawn from standard normal distribution"), 3, outer=T, cex=0.9, line=0.5, font=1)
...
The data points are taken from the center of a probability range (visualized by the checkerboard background in the graph), so all data points represent exactly the same fraction of the total probability range.

To validate that we didn't introduce another bias by our approach we also used repeated random drawings. This was done by taking the mean of 1000 random drawings from the standard normal distribution with different sample size. This is like taking the last value of our first graph and repeating the process for other sample sizes. We ended up with two very similar graphs:
...
mtext("probabilities of the theoretical quantiles", 4, line=4.5, outer=T)  
...
The remaining methods all underestimate the spread of the outer quantiles ("the extremes") at low sample sizes. The systematic bias gets smaller with increasing sample size. The shrinking of the bias happens later (at larger sample sizes) the more to the extremes the quantile is located.
...
Type 7 approaches the true value conservatively, meaning that it never overestimates the spread of the quantiles. In the contrary it tends to underestimate the spread of the outer quantiles. Everything below $$q_{0.125}$$ and above $$q_{0.875}$$ is noticeable biased for  $$n<50$$.

Type 6, 8 and 9 show overshooting in the estimation of the outer quantiles. The overshooting phase happens later for quantiles located more at the extremes. Most severe is this overshooting for type 6. 
The overshooting is more visible in the discretized version than in the graph deduced from random drawings. This is also evident in the next graphs. 
...
We already established that the outer quantiles show higher deviation from the expected value and loose this systematic bias later (at higher sample sizes). The next graphs show the deviation of the estimated quantiles from the expected values. They show the whole probability range for some selected sample sizes.

```{r difference-theoretical-to-estimated-quantiles, dev="svg"}
par(mfrow=c(3,3), mar=c(0,1,2,.1), oma=c(5,3,5,6))
...
mtext(paste0("using a discretized standard normal distribution as input"), 3, outer=T, line=0.5, font=1, cex=1)
mtext("deviation from expected quantiles", 2, outer=T, line=2)
...
mtext(paste0("systematic deviation from theoretical quantiles with variation of n"), 3, outer=T, line=3.9, font=2, cex=1)
mtext(paste0("using random samples drawn from a standard normal distribution"), 3, outer=T, line=2.2, font=1, cex=0.9)
mtext("(mean of 1000 estimations)", 3, outer=T, cex=0.9, line=0.5, font=1)
mtext("deviation from theoretical quantiles", 2, outer=T, line=2)
mtext("probabilities", 1, outer=T, line=3)
```

We can see that the estimations of methods 1 to 3 are not only erratic depending on what sample size is choosen, but also depending on the quantile that is calculated. Again they don't look like they should be used.
...
From these graphs we can learn how the deviation from the expected values is extremely more severe for the outer quantiles. But also how fast the deviation disappears for the inner quantiles with increasing sample size. This leads us to our next question about which quantiles can be calculated with tolerable bias for which sample size – without having to expect the large systematic deviations shown in the above graphs.


# Which quantiles can be estimated with relatively low systematic bias? 

Systematic deviations between the estimated quantiles and their expected value are evident from the graphs shown so far. To minimize these systematic deviations one could define a threshold, for example $$0.1\sigma$$. We then can calculate for which quantile deviations start to go well beyond this threshold. We end up with a collection of maximum probabilities that are safe to calculate quantiles for. Of course these threshold probabilities again are dependend on sample size. Since type 7 is the R default and type 5 is the best performing so far we show their difference exemplary for all other methods. We will work here with the results derived from random drawings.

```{r deviation-greater-than-tolerance-type-5, dev="svg", fig.height=6}
par(mfrow=c(2,1), mar=c(3,1,2,.1), oma=c(2,4,3,6), cex.main=1)
...
```{r type5-Hazen-and-type7-Weibull}
n=1000
pb_i <- seq(1/(n*2), 1-1/(n*2), length.out=n)
qb_i <- qnorm(pb_i)
plot(pb_i, qb_i, type="l")

n = 20
p_i <- seq(1/(n*2), 1-1/(n*2), length.out=n)
q_i <- qnorm(p_i)

der <- approx(y = q_i, x = p_i, xout=pb_i, yleft = q_i[1], yright = tail(q_i,1))

lines(y~x, der, col="red")
points(p_i, q_i, col="red")
...
lines(y~x, der, col="green")
points(p_i, q_i, col="green")
```

```{r type5-Hazen-recreated}
for(n in c(2,3,5,10,20,50)){
  i = 1:n
  p_i <- (i-1/2)/n
  q_i <- qnorm(p_i)
  ...
  if(n==2){
    plot(pb_i, der$y - qb_i, type="l", ylim=c(-0.4, 0.4))
  }else{
    lines(pb_i, der$y - qb_i)
  }
}
abline(h=0)
...
for(n in c(2,3,5,10,20,50)){
  ...
  der <- approx(y = q_i, x = p_ii, xout=pb_i, yleft = q_i[1], yright = tail(q_i,1))
...
for(k in 1:n){
  ...
  curve(factorial(n)/(factorial(k-1)*factorial(n-k)) * (x)^(k-1) * (1-x)^(n-k), add = T)
  
  #curve(3* (x)^(k-1) * (1-x)^(n-k), add=T)
  
  
  #curve(12*(x)^2*(1-x), add=T) # n=4, k=n-1=3
  #curve(20*(x)^3*(1-x), add=T) # n=5, k=n-1=4
  #curve(30*(x)^2*(1-x)^2, add=T) # n=5, k=n-2=3
  #curve(20*(x)^1*(1-x)^3, add=T) # n=5, k=n-3=2
  #curve(30*(x)^1*(1-x)^4, add=T) # n=6, k=n-4=2
  #curve(60*(x)^2*(1-x)^3, add=T) # n=6, k=n-3=3
  abline(v=(k-1/2)/n)
  abline(v=k/(n+1), lty=2)
  abline(v=median(res), col="red", lwd=2)
  abline(v=mean(res), col="green", lwd=2)
}
...
# curve(n*(1-x)^(n-1), add=T) # k=1
# curve(2*(1-x), add=T) # n=2, k=1
# curve(3*(1-x)^2, add=T) # n=3, k=1
# curve(4*(1-x)^3, add=T) # n = 4, k=1
# curve(5*(1-x)^4, add=T) # n = 5, k=1
# 
# curve(n*(x)^(n-1), add=T) # k=n
# curve(2*x, add=T) # n=2, k=n
# curve(3*(x)^2, add=T) # n=3, k=n
# curve(4*x^3, add=T) # n = 4, k=n
# curve(5*x^4, add=T) # n = 5, k=n
...
```
```{r Weibull-and-Gumbel-argument_plotted-together}
N <- 10000
n <- 10
plot(NULL, xlim=c(0,1), ylim=c(0,6), xlab="probability", ylab="density")
for(k in 1:n){
  curve(factorial(n)/(factorial(k-1)*factorial(n-k)) * (x)^(k-1) * (1-x)^(n-k), add = T)
  abline(v=k/(n+1), lty=2, col="gray")
}
  abline(v=median(res), col="red", lwd=2)
```
...
```{r Weibull-and-Gumbel-argument_plotted-together-quantiles}
N <- 10000
n <- 10

d <- 2^8-1
p <- seq(1/(d+1), 1-1/(d+1), length.out=d)
q <- qnorm(p)
plot(NULL, xlim=range(q), ylim=c(0,6), xlab="quantiles", ylab="density")

for(k in 1:n){
  i=1
  res <- rep(NA, N)
  while(i <= N){
    rr <- rnorm(n, mean=0, sd = 1)
    res[i] <- sort(rr)[k]
    
    i = i+1
  }
  
  #hist(res, freq = F, breaks = seq(-6,6, length.out=80), border ="gray")
  
  lines(q, factorial(n)/(factorial(k-1)*factorial(n-k)) * (p)^(k-1) * (1-p)^(n-k))
  abline(v=qnorm(k/(n+1)), lty=2, col="gray")
}
  #abline(v=median(res), col="red", lwd=2)
```
...
N <- 100000
n <- 10
diff_weibull <- rep(NA, n)
diff_hazen <- rep(NA, n)
plot(NULL, xlim=c(-2,2), ylim=c(0,1.2), xlab="quantiles", ylab="density")
#plot(NULL, xlim=c(0,1), ylim=c(0,4), xlab="quantiles", ylab="density")
for(k in 1:n){
  #k=2
  i=1
  res <- rep(NA, N)
  ...
  abline(v=qnorm((k-1/2)/n)) # Hazen (solid)
  abline(v=qnorm(k/(n+1)), lty=2) # Weibull (striped)
  abline(v=qnorm((k-0.4)/(n+0.2)), lty=3) # Cunnanue
  abline(v=qnorm((k-0.375)/(n+0.25)), lty=4) # Blom
  abline(v=median(res), col="red", lwd=2)
  abline(v=mean(res), col="green", lwd=2)
  # diff_weibull[k] <- mean(res) - qnorm(k/(n+1))
  # diff_hazen[k] <- mean(res) - qnorm((k-1/2)/n) 
}
legend("topright", lty=c(1,2,3, 4,1,1), col=c("black", "black","black","black","red", "green"), legend=c("Hazen (5)", "Weibull (6)", "Cunnane" ,"Blom (9)", "Median", "Mean"))

# plot(1:n/n, diff_weibull, type="l")
# lines(1:n/n, diff_hazen, col="green")

```
```{r repeated-QuantileSamples}
#nn <- c(2:9, signif(10^seq(1, 3, 1/3), 1))
#nn2 <- c(2,3,5,10,20,50)
nn <- 10
...
QuantileSamples <- array(NA, dim = c(6, repetitions, length(p), length(nn)), dimnames = list(type=4:9, repetitions=1:repetitions, p=p, n=nn))

library(R.utils)
pb <- ProgressBar(max = repetitions)
reset(pb)
for(n in as.character(nn)){
  for(type in 4:9){
    #set.seed(1)
    for(i in 1:repetitions){
      cat("type: ", type, " " , format(i, big.mark = " "), "         \r")
      QuantileSamples[as.character(type),i,,n] <-  quantile(rnorm(n), p, type=type)
    }
  }
...
```{r scatterplot}
plot(p_i.fine, qnorm(p_i.fine), type="l")
lines(p, QuantileSamples[7, 1, , 1], type="l")
```
```{r}
seq_along(p)/(n+1)
```
...
n=10
for(type in 4:9){
  plot(NULL, xlim=c(-3,3), ylim=c(0,1.2), xlab="quantiles", ylab="density", main=paste("type =", type))
  for(i in seq_along(p)){
    lines(density(QuantileSamples[as.character(type), ,i, 1]))
    #abline(v=qnorm((i-1/2)/n))
    abline(v=mean(QuantileSamples[as.character(type), ,i, 1]), col="green")
    #abline(v=median(QuantileSamples[7, ,i, 1]), col="blue")
    ##abline(v=qnorm(i/(n+1)), lty=2)
  }
  abline(v=qnorm(p), col="red")
}
...
We can see that for method type 5 the outer quantiles (for example $$q<q_{0.05}$$ and $$q>q_{0.95}$$ become calculable with bias less than $$0.1\sigma$$ for far lower sample sizes than is the case for type 7. While type 5 allows the estimation already for $$n=10$$ we have to provide $$n=50$$ to method 7 to get a comparable bias.
...
- Harter 1984 ->
- "Bringing Closure to the Plotting Position Controversy" (MAKKONEN 2008) favourises type 6.
