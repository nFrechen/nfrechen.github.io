...
mtext("theoretical probabilities corresponding to the quantiles", 4, line=4.5, outer=T)  
...
mtext(paste0("systematic deviation of estimated from theoretical quantiles"), 3, outer=T, line=1, font=2, cex=1)
...
mtext(paste0("systematic deviation from theoretical quantiles with varying n"), 3, outer=T, line=1, font=2, cex=1)
mtext("deviation from theoretical quantiles", 2, outer=T, line=3)
mtext("probabilities", 1, outer=T, line=3)
palette("default")
...
par(mfrow=c(1,2), mar=c(0,1,2,.1), oma=c(5,4,3,6))
...
plot(NULL, xlim=c(1,nmax), ylim=ylim, log="x", 
     main='"quantiles" calculated to display boxplots', xlab="", ylab="", frame.plot=T, panel.first=abline(v=c(1,2,5,10,20,50,100,200,500,1000), h=qnorm(Q), col="lightgray"))
for(i in 1:5){
  lines(Qmatrix[,i], col=cols[i], type="l")
}
axis(4, at=qnorm(Q), Q, las=1)
legend("topleft", legend = rev(colnames(Qmatrix)), col=cols, lty=1)
mtext("theoretical probabilities corresponding to the quantiles", 4, line=4.5, outer=T)  
mtext("estimated quantile", side = 2, outer = T, cex=1, line=2.5)
mtext("number of points uniformly distributed over probabilities",1, outer=T, line=3.5)

# plot example boxplot
bxp(boxplot(qnorm(x), plot = F), medcol=3, staplecol=1, boxcol=2, lwd=2, ylim=ylim, axes=F)
