...
[Since 2004](https://stat.ethz.ch/pipermail/r-announce/2004/000427.html) R has implemented 9 types of quantile estimation recommended by Hyndman and Fan (1996):
...
>
> Type&nbsp;4: | Parzen&nbsp;(1979) | $$m = 0$$ | $$p_k = \frac{k}{n}$$ | That is, linear interpolation of the empirical cdf.
> Type&nbsp;5: | Hazen&nbsp;(1914) | $$m = \frac{1}{2}$$ | $$p_k = \frac{k - 0.5}{n}$$ | That is a piecewise linear function where the knots are the values midway through the steps of the empirical cdf. This is popular amongst hydrologists.
> Type&nbsp;6: | Weibull&nbsp;(1939), Gumbel&nbsp;(1939) | $$m = p$$ | $$p_k = \frac{k}{n + 1}$$ | Thus $$p_k = E[F(x[k])]$$. This is used by Minitab and by SPSS.
> Type&nbsp;7: | Gumbel&nbsp;(1939) | $$m = 1-p$$ |  $$p_k = \frac{k - 1}{n - 1}$$ | In this case, $$p_k = mode[F(x[k])]$$. This is used by S and by R < 2.0.0.
> Type&nbsp;8: | Johnson and Kotz (1970) |$$m = \frac{p+1}{3}$$ | $$p_k = \frac{k - 1/3}{n + 1/3}$$ | Then $$p_k \approx median[F(x[k])]$$. The resulting quantile estimates are approximately median-unbiased regardless of the distribution of $$x$$.
> Type&nbsp;9: | Blom&nbsp;(1958) | $$m = \frac{p}{4} + \frac{3}{8}$$ | $$p_k = \frac{k - 3/8}{n + 1/4}$$ | The resulting quantile estimates are approximately unbiased for the expected order statistics if $$x$$ is normally distributed.
>
...
```{r plotting-positions, fig.cap="Visualisation of plotting positions $p_k$ for a dataset of 10 values, as recommended by different authors", fig.height=4.5, dev="tikz", echo=F, fig.width=8, fig.show="hide"}
...
par(mar=c(0,1,2,4), oma=c(4,3,3,1))
...
plot(NULL, xlim=c(1,nmax), ylim=ylim, log="x", 
     main='"quantiles" calculated to draw boxplots', xlab="", ylab="", frame.plot=T, panel.first=abline(v=c(1,2,5,10,20,50,100,200,500,1000), h=qnorm(Q), col="lightgray"))
for(i in 1:5){
  lines(Qmatrix[,i], col=cols[i], type="l")
}
axis(4, at=qnorm(Q), Q, las=1)
legend("topleft", legend = rev(colnames(Qmatrix)), col=cols, lty=1, bg="white")
mtext("probabilities corresponding to theoretical quantiles", 4, line=0, outer=T)  
mtext("estimated quantiles", side = 2, outer = T, cex=1, line=1.5)
mtext("number of points uniformly distributed over probabilities",1, outer=T, line=2.5)

# plot example boxplot
bxp(boxplot(qnorm(x), plot = F), medcol=3, staplecol=1, boxcol=2, lwd=2, ylim=ylim, axes=F, add=T, at=1000)
