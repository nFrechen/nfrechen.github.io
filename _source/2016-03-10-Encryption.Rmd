...
library(knitr)
opts_chunk$set(message = F, error = F, warning = F, cache = TRUE, autodep=T, fig.width=7, fig.height=5, dev="svg", dpi=100, tidy=T)
...
To keep it simple we translate all to uppercase letters:
...
And we replace the spacing between words with _:
```{r}
mes <- gsub(" ", "_", mes)
```
and get "`r mes`".
...
We achieve this in R by writing:
```{r, results="hide"}
mes_enc <- key[mes]
```

To get a single string we paste the letters in the vector together like this:
```{r, eval=T}
...
This becomes more obvious if we have alonger message:
```{r}
...
# download the book
text <- readLines("https://archive.org/download/adventuresoftoms00twai2/adventuresoftoms00twai2_djvu.txt")
...
```{r sorted-word-frequency, echo=F}
...
```{r, eval=F}
base=75
mes_num_enc <- encrypt(text = mes_num, key = 5, base = base)

mes_num_dec <- encrypt(mes_num_enc, key=19, base=base)
mes_num_dec
mes_num
```
