...
Allerdings stellt sich so eine Erkenntnis nicht unbedingt durch das Betrachten einer einzelnen Grafik ein. Meistens bedarf es einiges an Vorwissen: Um welche Daten handelt es sich? Wie wurden die Daten für die Grafik vorbereitet? Welche Darstellungsform wurde gewählt? Manchmal ist eine Grafik allein auch nicht ausreichend, um die Daten erschöpfend zu beschreiben und verständlich zu machen. Es müssen also meist mehrere Grafiken in einen erzählerischen Zusammenhang gebracht werden. Und ohne etwas Text wird das Ganze wahrscheinlich auch nicht auskommen.

Daher soll in diesem Blogpost versucht werden, Daten durch eine solche grafische Erzählung erfahrbar zu machen. Im Folgenden soll also möglichst wenig Text verwendet werden. Der Text soll hauptsächlich auf Charakteristiken in den Grafiken aufmerksam machen und damit den Erkenntnisgewinn etwas leiten. Durch Interaktivität in den Grafiken soll ein mehr explorativer, also erforschender Erkenntnisgewinn ermöglicht werden und die Linearität der Erzählung durchbrochen werden, indem viele verschiedene Wege gewählt werden können, um die Daten zu verstehen.
