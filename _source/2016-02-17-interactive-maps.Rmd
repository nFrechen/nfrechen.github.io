...
![map tile](http://a.tile.openstreetmap.org/3/4/2.png)
...
## Web Map Service (WMS)
The call for a Web Map Service is a bit more complex, but you also get additional features. According to [Wikipedia](https://en.wikipedia.org/wiki/Web_Map_Service#Requests):
...
The call is not done with a simple url. So we leave it to libraries like leafletjs to do the call for us.

# Use leaflet maps in R

Let's start by installing the package and load it:
...
Then just type `leaflet()` to create a plain leaflet map and add a openstreetmap map with `addTiles()`.
