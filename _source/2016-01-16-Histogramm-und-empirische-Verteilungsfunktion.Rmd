...
opts_chunk$set(message = F, error = F, warning = F, cache=T, autodep = T, fig.width = 7, fig.height = 3.5, dev="svg", small.mar=T)
```
...
```{r empirische-Verteilung-temp-Juni}
...
```
```{r Schiefe-Verteilung-not, eval=F, echo=F}
X <- rep(0, n)
add = 0.1
for(i in 1:n){
	for(j in 1:decisions){
		if(add==0.0001){
			add <- sample(c(0.0001,0.1), prob = c(10,1), size=1)
		}else{
			add <- sample(c(0.0001,0.1), prob = c(1,1), size=1)
		}
		X[i] <- X[i] + add
	}
}
...
```{r Auswertung-Bodensaeule, dependson="Modell Bodensaeule", cache=F, fig.height=4}
par(mar=c(4,4,2,4))
...
```{r Verteilungen, echo=F}
curve( dweibull(x, shape = 3, scale = 2), n = 300, col="red", lwd=3, from = 0, to = 10, ylab="Density")
curve( dlnorm(x, meanlog = 1, sdlog = 1), add=T, n = 300, lwd=3)
curve( dgamma(x, shape = 2, rate=1, scale=1), n = 300, col="green", add=T, lwd=3)

legend("topright", c("Weibull", "log-Normal", "Gamma"), title="Verteilung", col=c("red", "black", "green"), lwd=3)
```
